...
import (
	...
	ep "git.bluebird.id/mini/customer/endpoint"
	pb "git.bluebird.id/mini/customer/grpc"
	svc "git.bluebird.id/mini/customer/server"

	cfg "git.bluebird.id/mini/util/config"
	run "git.bluebird.id/mini/util/grpc"
...
func main() {
	...
	var server pb.CustomerServiceServer
	{
		...
		server = ep.NewGRPCCustomerServer(endpoint, tracer, logger)
	}
