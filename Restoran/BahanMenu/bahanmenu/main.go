...
func main() {
	...
	var server pb.BahanMenuServiceServer
	{
		...
		service := svc.NewBahanMenu(dbReadWriter)
		endpoint := ep.NewBahanMenuEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCBahanMenuServer(endpoint, tracer, logger)
		fmt.Println("tes server", server)
	}
	grpcServer := grpc.NewServer(run.Recovery(logger)...)
	pb.RegisterBahanMenuServiceServer(grpcServer, server)
