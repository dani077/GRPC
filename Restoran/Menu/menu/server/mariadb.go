...
const (
	...
	selectMenu = `select IDMenu,namaMenu,harga,IDKategoriMenu,createdBy,status,createdOn 
		from tbMenu `
	...
	selectNamaMenu = `select IDMenu,namaMenu,harga,IDKategoriMenu,createdBy,status 
		from tbMenu where namaMenu like ?`
	selectMenuketerangan = `select IDMenu,namaMenu,harga,IDKategoriMenu,createdBy,status,keterangan
		from tbMenu where keterangan LIKE ?`
)
...
func (rw *dbReadWriter) ReadMenu() (Menus, error) {
	...
	for rows.Next() {
		var m Menu
		err := rows.Scan(&m.IDMenu, &m.NamaMenu, &m.Harga, &m.IDKategoriMenu, &m.CreatedBy, &m.Status, &m.CreatedOn)
		if err != nil {
...
func (rw *dbReadWriter) ReadMenuByKeteranganMenu(keterangan string) (Menus, error) {
	fmt.Println("read by keterangan berhasil")
	menu := Menus{}
	rows, _ := rw.db.Query(selectMenuketerangan, keterangan)
	defer rows.Close()
	for rows.Next() {
		var m Menu
		err := rows.Scan(&m.IDMenu, &m.NamaMenu, &m.Harga, &m.IDKategoriMenu, &m.CreatedBy, &m.Status, &m.Keterangan)
		if err != nil {
			fmt.Println("error query:", err)
			return menu, err
		}
