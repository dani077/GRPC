...
import (
	...
	pb "git.bluebird.id/Restoran/Menu/menu/grpc"
	svc "git.bluebird.id/Restoran/Menu/menu/server"

	util "git.bluebird.id/Restoran/Menu/util/grpc"
	disc "git.bluebird.id/Restoran/Menu/util/microservice"
...
func NewGRPCMenuClient(nodes []string, creds credentials.TransportCredentials, option util.ClientOption,
	...
	var readMenuByKeteranganMenuEp endpoint.Endpoint
	{
		factory := util.EndpointFactory(makeClientReadMenuByKeteranganMenuEndpoint, creds, timeout, tracer, logger)
		endpointer := sd.NewEndpointer(instancer, factory, logger)
		balancer := lb.NewRoundRobin(endpointer)
		retry := lb.Retry(retryMax, retryTimeout, balancer)
		readMenuByKeteranganMenuEp = retry
	}

	return MenuEndpoint{AddMenuEndpoint: addMenuEp, ReadMenuByNamaMenuEndpoint: readMenuByNamaMenuEp, ReadMenuEndpoint: readMenuEp, UpdateMenuEndpoint: updateMenuEp, ReadMenuByKeteranganMenuEndpoint: readMenuByKeteranganMenuEp}, nil
}
...
func encodeReadMenuByKeteranganMenuRequest(_ context.Context, request interface{}) (interface{}, error) {
	req := request.(svc.Menu)
	return &pb.ReadMenuByKeteranganMenuReq{Keterangan: req.Keterangan}, nil
}
...
func decodeReadMenuByKeteranganMenuResponse(_ context.Context, response interface{}) (interface{}, error) {
	resp := response.(*pb.ReadMenuByKeteranganMenuResp)
	var rsp svc.Menus

	for _, v := range resp.AllKeteranganMenu {
		itm := svc.Menu{
			IDMenu:         v.Idmenu,
			NamaMenu:       v.Namamenu,
			Harga:          v.Harga,
...
func makeClientReadMenuByKeteranganMenuEndpoint(conn *grpc.ClientConn, timeout time.Duration, tracer stdopentracing.Tracer,
	logger log.Logger) endpoint.Endpoint {

	endpoint := grpctransport.NewClient(
		conn, grpcName,
		"ReadMenuByKeteranganMenu",
		encodeReadMenuByKeteranganMenuRequest,
		decodeReadMenuByKeteranganMenuResponse,
		pb.ReadMenuByKeteranganMenuResp{},
		grpctransport.ClientBefore(opentracing.ContextToGRPC(tracer, logger)),
	).Endpoint()

	endpoint = opentracing.TraceClient(tracer, "ReadMenuByKeteranganMenu")(endpoint)
	endpoint = circuitbreaker.Gobreaker(gobreaker.NewCircuitBreaker(gobreaker.Settings{
		Name:    "ReadMenuByKeteranganMenu",
		Timeout: timeout,
	}))(endpoint)
