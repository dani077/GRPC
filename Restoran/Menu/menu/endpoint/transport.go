...
import (
	...
	scv "git.bluebird.id/Restoran/Menu/menu/server"
...
type grpcMenuServer struct {
	...
	readMenuByKeteranganMenu grpctransport.Handler
}
...
func NewGRPCMenuServer(endpoints MenuEndpoint, tracer stdopentracing.Tracer,
	...
	return &grpcMenuServer{
		...
		readMenuByKeteranganMenu: grpctransport.NewServer(endpoints.ReadMenuByKeteranganMenuEndpoint,
			decodeReadMenuByKeteranganMenuRequest,
			encodeReadMenuByKeteranganMenuResponse,
			append(options, grpctransport.ServerBefore(opentracing.GRPCToContext(tracer, "ReadMenuByKeteranganMenu", logger)))...),
	}
}
...
func decodeReadMenuByKeteranganMenuRequest(_ context.Context, request interface{}) (interface{}, error) {
	req := request.(*pb.ReadMenuByKeteranganMenuReq)
	return scv.Menu{Keterangan: req.Keterangan}, nil
}
...
func encodeReadMenuByKeteranganMenuResponse(_ context.Context, response interface{}) (interface{}, error) {
	resp := response.(scv.Menus)

	rsp := &pb.ReadMenuByKeteranganMenuResp{}
	...
	for _, v := range resp {
		...
		rsp.AllKeteranganMenu = append(rsp.AllKeteranganMenu, itm)
	}
	return rsp, nil
}
...
func (s *grpcMenuServer) ReadMenuByKeteranganMenu(ctx oldcontext.Context, keterangan *pb.ReadMenuByKeteranganMenuReq) (*pb.ReadMenuByKeteranganMenuResp, error) {
	_, resp, err := s.readMenuByKeteranganMenu.ServeGRPC(ctx, keterangan)
	if err != nil {
		return nil, err
	}
	return resp.(*pb.ReadMenuByKeteranganMenuResp), nil
}
